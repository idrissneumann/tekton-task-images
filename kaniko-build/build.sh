...
if [ "${GIT_BRANCH}" != "develop" ] && [ "${GIT_BRANCH}" != "main" ] && [ "${GIT_BRANCH}" != "master" ] && ! echo "${GIT_BRANCH}"|grep -E "^[0-9]+.[0-9]+.x$" > /dev/null || [ "${FORCE_PROJECT_CI}" == "enabled" ]; then
    DOCKER_REGISTRY_ORG=$PROJECT_CI
fi
...
if [ "${ENABLE_CACHE}" == "enabled" ] || [ "${ENABLE_CACHE}" == "ok" ] || [ "${ENABLE_CACHE}" == "true" ] || [ "${ENABLE_CACHE}" == "enabled" ]; then
    CACHE_OPTS="--cache=true --cache-ttl=${CACHE_TTL} --cache-repo=${DOCKER_REGISTRY}/${PROJECT_CI}/cache --cache-dir=$TEKTON_WORKSPACE_PATH"
fi
